...
.data
    szDisplayName db "NTU Soccer",0     ;DD:Define Double Word，要用DWORD也可
    AppName db "NTU Soccer", 0
    ...
    ballBounce db "sounds/ballBouncing.mp3", 0
...
; _____________________________________________________________________________________________________
    ...
    movePlayer proc uses eax addrPlayer:dword
        ...
        ; 如果玩家向上跳，它會從地面“下降”
        .if eax >= 420
            mov [edx].jumping, FALSE ;我們警告你他不再跳躍
...
; _____________________________________________________________________________________________________
    ...
    moveBall proc uses eax addrBall:dword
        ...
        .if [ebx].ballObj.pos.y < 437       ;如果球在空中，我們拉它（重力）
            mov ecx, [ebx].ballObj.speed.y
            inc ecx
            mov [ebx].ballObj.speed.y, ecx
        .endif

        .if [ebx].ballObj.pos.y >= 437                      ; 如果球碰到地面，讓我們讓它反彈    
            ;mov edx, 0
    ...
    ballColliding proc
        ...
        .if edx == TRUE  && ecx == TRUE                      ; 相撞
            ...
            .if eax == 0                                    ; 如果玩家是靜止的
                ...
                neg eax
            .else                                           ; 如果玩家在移動
                add eax, player1.playerObj.speed.x          ; 我們根據你的速度踢
                .if ultimate_player1 == 1
                    add eax, 50
                    mov ultimate_player1, 0
                .endif
...
; _____________________________________________________________________________________________________
    ...
    WinMain proc hInst     :DWORD,
        ...
        mov wc.lpszClassName,  offset szClassName  ; window class name ;視窗結構體的名稱 ;給視窗結構體命名，CreateWindow函式將根據視窗結構體名稱來建立視窗
        ; RC 文件中的圖標 ID
